...
pub struct GeminiMcpHttp {
    ...
    pub http_url: Option<String>, // HTTP endpoint
    #[serde(default)]
...
/// Parsed MCP from Gemini format (normalized to internal format)
#[derive(Debug)]
pub struct ParsedGeminiMcp {
    pub name: String,
    pub mcp_type: String, // "stdio", "http", or "sse"
...
/// Gemini settings.json structure (uses mcpServers key like Claude Code)
...
/// Parse Gemini settings.json file and extract MCP servers
pub fn parse_gemini_mcps(path: &Path) -> Result<Vec<ParsedGeminiMcp>> {
    let content = std::fs::read_to_string(path)?;
    let config: GeminiSettingsConfig = serde_json::from_str(&content)?;
...
fn parse_mcp_entry(name: &str, config: &Value) -> Result<ParsedGeminiMcp> {
    let obj = config
        .as_object()
        .ok_or_else(|| anyhow::anyhow!("MCP config entry is not an object"))?;

    // Determine type based on presence of url/httpUrl vs command
    let has_url = obj.contains_key("url");
    let has_http_url = obj.contains_key("httpUrl");
    let has_command = obj.contains_key("command");

    if has_http_url || has_url {
        // HTTP/SSE transport
        // Prefer httpUrl over url for the primary URL
        let url = obj
            .get("httpUrl")
            .or_else(|| obj.get("url"))
            .and_then(|v| v.as_str())
        ...
        // Determine type based on which field was used
        let mcp_type = if has_http_url { "http" } else { "sse" };

        // Gemini uses direct headers
...
pub fn write_gemini_config(path: &Path, mcps: &[McpTuple]) -> Result<()> {
    ...
    for mcp in mcps {
        ...
        match mcp_type.as_str() {
            ...
            "http" => {
                // Use httpUrl for HTTP type
                if let Some(u) = url {
                    server_obj.insert("httpUrl".to_string(), Value::String(u.clone()));
                }
...
pub fn add_mcp_to_gemini_config(path: &Path, mcp: &McpTuple) -> Result<()> {
    ...
    // Write all MCPs
    write_gemini_config(path, &all_mcps)
}
...
pub fn remove_mcp_from_gemini_config(path: &Path, name: &str) -> Result<()> {
    ...
    let content = std::fs::read_to_string(path)?;
    let mut config: GeminiSettingsConfig = serde_json::from_str(&content)?;
...
mod tests {
    ...
    // =========================================================================
    // parse_gemini_mcps tests
    // =========================================================================

    #[test]
    fn test_parse_gemini_mcps_stdio() {
    ...
    #[test]
    fn test_parse_gemini_mcps_http_url() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("settings.json");
    ...
    #[test]
    fn test_parse_gemini_mcps_sse_url() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("settings.json");
        ...
        let mcp = &mcps[0];
        assert_eq!(mcp.name, "streaming");
        assert_eq!(mcp.mcp_type, "sse");
        assert_eq!(mcp.url, Some("https://api.example.com/sse".to_string()));
    }

    #[test]
    fn test_parse_gemini_mcps_multiple() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("settings.json");
    ...
    #[test]
    fn test_parse_gemini_mcps_empty() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("settings.json");
    ...
    #[test]
    fn test_parse_gemini_mcps_no_mcp_servers() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("settings.json");
    ...
    // =========================================================================
    // write_gemini_config tests
    // =========================================================================

    #[test]
    fn test_write_gemini_config_stdio() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("settings.json");
    ...
    #[test]
    fn test_write_gemini_config_http() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("settings.json");
        ...
        assert!(content.contains("\"httpUrl\": \"https://api.example.com\""));
        assert!(content.contains("\"headers\""));
    }

    #[test]
    fn test_write_gemini_config_sse() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("settings.json");
        ...
        let content = fs::read_to_string(&config_path).unwrap();
        assert!(content.contains("\"sse-mcp\""));
        assert!(content.contains("\"url\": \"https://api.example.com/sse\""));
    }

    #[test]
    fn test_write_gemini_config_preserves_other_content() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("settings.json");
    ...
    #[test]
    fn test_write_gemini_config_creates_parent_dirs() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("subdir").join("settings.json");
    ...
    // =========================================================================
    // add_mcp_to_gemini_config tests
    // =========================================================================

    #[test]
    fn test_add_mcp_to_gemini_config() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("settings.json");
        ...
        add_mcp_to_gemini_config(&config_path, &mcp1).unwrap();
        ...
        add_mcp_to_gemini_config(&config_path, &mcp2).unwrap();
    ...
    // =========================================================================
    // remove_mcp_from_gemini_config tests
    // =========================================================================

    #[test]
    fn test_remove_mcp_from_gemini_config() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("settings.json");
