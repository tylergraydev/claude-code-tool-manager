<script lang="ts">
	...
	async function handleSave(settings: ClaudeSettings) {
		try {
			...
			notifications.success('Sandbox settings saved');
		} catch (err) {
			notifications.error('Failed to save sandbox settings');
		}
	}
...
<Header
	title="Sandbox"
	subtitle="Configure bash command isolation, network access, and security settings"
/>
