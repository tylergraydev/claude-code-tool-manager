<script lang="ts">
	...
	async function handleSaveModelConfig(settings: ClaudeSettings) {
		try {
			await claudeSettingsLibrary.save(settings);
			notifications.success('Model settings saved');
		} catch (err) {
			notifications.error('Failed to save model settings');
		}
	}

	async function handleSaveAttribution(settings: ClaudeSettings) {
		try {
			await claudeSettingsLibrary.save(settings);
			notifications.success('Attribution settings saved');
		} catch (err) {
			notifications.error('Failed to save attribution settings');
		}
	}
...
<Header
	title="Models & Output"
	subtitle="Configure model defaults, output behavior, and git attribution"
/>
