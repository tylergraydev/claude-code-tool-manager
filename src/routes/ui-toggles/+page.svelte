<script lang="ts">
	...
	async function handleSave(settings: ClaudeSettings) {
		try {
			...
			notifications.success('UI toggle settings saved');
		} catch (err) {
			notifications.error('Failed to save UI toggle settings');
		}
	}
...
<Header
	title="UI Toggles"
	subtitle="Control visual and interaction preferences for Claude Code"
/>
